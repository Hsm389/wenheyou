...
public static class Drawing_tc
{
    ...
    {
       	BezierLine(
            new Vector2(wr.x + wr.width, wr.y + width + wr.height / 2),
            new Vector2(wr.x + wr.width + Mathf.Abs(wr2.x - (wr.x + wr.width)) / 2, wr.y + width + wr.height / 2),
            new Vector2(wr2.x, wr2.y + width + wr2.height / 2),
            new Vector2(wr2.x - Mathf.Abs(wr2.x - (wr.x + wr.width)) / 2, wr2.y + width + wr2.height / 2), shadow, width, true,20,screen);
        BezierLine(
            ...
            new Vector2(wr2.x - Mathf.Abs(wr2.x - (wr.x + wr.width)) / 2, wr2.y + wr2.height / 2), color, width, true,20,screen);
    }
	...
	public static void BezierLine(Vector2 start, Vector2 startTangent, Vector2 end, Vector2 endTangent, Color color, float width, bool antiAlias, int segments,Rect screen)
    {
        ...
        for (int i = 1; i <= segments; ++i)
        {
            ...
            Drawing_tc.DrawLine(lastV, v, color, width, antiAlias,screen);
    ...
    private static Matrix4x4 translationMatrix(Vector3 v)
    {
        ...
        return Matrix4x4.TRS(v, Quaternion.identity, Vector3.one);
             ...
             */
			static bool clippingEnabled = true;
            static Rect clippingBounds;
            public static Material lineMaterial = null;
 
             /* @ Credit: "http://cs-people.bu.edu/jalon/cs480/Oct11Lab/clip.c" */
            static bool clip_test(float p, float q, ref float u1, ref float u2)
            {
                float r;
                bool retval = true;
            ...
            static bool segment_rect_intersection(Rect bounds, ref Vector2 p1, ref Vector2 p2)
            {
            ...
            public static void DrawLine(Vector2 start, Vector2 end, Color color, float width)
            {
                ...
                if (width == 1) {
                    ...
                    Vector3 perpendicular = (startPt - endPt).normalized * width/2f;
